...
async def discovery():
    ...
    try:
        ...
        for device in discovered_devices:
            ...
            cfg_devices.set('Midea_' + str(device.id),"modes", [str(mode.name) for mode in device.supported_operation_modes])
            cfg_devices.set('Midea_' + str(device.id),"swingmodes", [str(swingmode.name) for swingmode in device.supported_swing_modes])
