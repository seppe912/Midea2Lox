...
def discover(debug: int):
    ...
    found_devices = {}
    discover_time = 10
    _LOGGER.debug("msmart version: {}".format(VERSION))
    _LOGGER.info(
        ...
        try:
            ...
            while (datetime.now()-t1).seconds  <= discover_time:
                ...
                if len(data) >= 104 and (data[:2].hex() == '5a5a' or data[8:10].hex() == '5a5a'):
                    ...
                    result = "*** Found a {} device - type: '0x{}' - version: {} - ip: {} - port: {} - id: {} - sn: {} - ssid: {}. *** ".format(m_support, m_type, m_version, m_ip, m_port, m_id, m_sn, m_ssid)
                    if m_version == "V3":
                        #result += "This device needs Token and K1, open https://github.com/mac-zhou/midea-ac-py/blob/master/README.md to see How to Get Token and K1."
                        result += "This device is supported but needs Token and K1, open https://www.loxwiki.eu/display/LOXBERRY/Midea2Lox to see How to Get Token and K1."
